...
uint global_enqueue_ticket = 0;

void mlfq_enqueue(struct proc *p, int priority) {
  p->priority = priority;
  p->enqueue_time = __sync_fetch_and_add(&global_enqueue_ticket, 1);
}
#endif
...
scheduler(void)
{
  ...
  for (;;) {
    ...
    int best_priority = 4;
    uint best_enqueue_time = 0xffffffff;

    for (p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock);
      if (p->state == RUNNABLE) {
        if (p->priority < best_priority || (p->priority == best_priority && p->enqueue_time < best_enqueue_time)) {
          best_priority = p->priority;
          best_enqueue_time = p->enqueue_time;
          best_p = p;
        }
      }
      release(&p->lock);
